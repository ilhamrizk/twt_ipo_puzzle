...
df_prices <- merge(x = read.csv('data_stock_prices - data_stock_prices.csv'),
                   y = df_sektor_all,
                   by.x = 'kode',
                   by.y = 'kode') %>%
...
df_bhar <- df_join %>%
  ...
  summarise(phi_bharit = prod(bharit),
            phi_bharbt = prod(bharbt)) %>%
  as.data.frame() %>%
  arrange('kode.x')

bhar250 <- df_bhar$phi_bharit - df_bhar$phi_bharbt
df_bhar$bhar250 <- bhar250
tail(df_bhar)

df_all <- read.csv('data_analysis_emiten_20230530.csv')
df_all_plus_bhar250 <- merge(x = df_all,
                          y = df_bhar,
                          by.x = 'kode',
                          by.y = 'kode.x',
                          all.x = TRUE)
tail(df_all_plus_bhar250)

write.csv(df_all_plus_bhar250, 'data_analysis_emiten_plusbhar250_20230530.csv')

df_summary <- as.data.frame(summary(df_all_plus_bhar250))
write.csv(df_summary, 'data_analysis_emiten_summary_plusbhar250_20230530.csv')
summary(df_all_plus_bhar250)

#View(df_all_plus_bhar250)
